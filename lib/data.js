...
const dataURL = "https://dev-week11-a11.pantheonsite.io/wp-json/twentytwentyone-child/v1/special";
...
export async function get_item_data(req_id) {
    ...
    jsonObj.forEach(
        (item) => {
            let reformattedString = '{"' + item.details + '"}';
            reformattedString = reformattedString.replaceAll(',','","');
            reformattedString = reformattedString.replaceAll(':','":"');

            let field_json = JSON.parse(reformattedString);
            console.log(field_json);

            item.details = field_json;
        }
    )
